...
Given a transient list, a directory of per-transient light-curve files and a
(pre-existing) tag, this walks the list and, for every entry:
...
The JWST Observatory -> Telescope -> Instrument chain is ensured up-front
(idempotently); everything is safe to re-run.

Usage:
    python scripts/ingest_transients_cli.py <transient_list> <lightcurve_dir> <tag>
    python scripts/ingest_transients_cli.py \\
        scripts/resources/jets_cands_labels.txt scripts/resources/ COSMOS

    # target / credentials come from the environment (or --url/--user/--pw):
    URL=http://localhost:8000 USER_YSE=djones PW=BossTent1 \\
        python scripts/ingest_transients_cli.py <list> <dir> <tag>
...
# Human-readable, stable names so the seeded chain is predictable in the UI.
OBSERVATORY_NAME = "Dev JWST Observatory"
TELESCOPE_NAME = "Dev JWST"
INSTRUMENT_NAME = "Dev NIRCam"
...
class Client:
    ...
    def get_or_create(self, list_path, body, match):
        """Return an existing object matching `match`, else create it.

        `match(item)` decides whether a listed object is the one we want.
        """
        page = self.get(f"{list_path}?limit=5000")
        for item in page.get("results", []):
            if match(item):
                return item, False
        resp = self.post(list_path, body)
        if resp.status_code != 201:
            raise RuntimeError(
                f"failed to create at {list_path}: HTTP {resp.status_code}\n{resp.text[:500]}"
            )
...
def ensure_instrument(client):
    """Idempotently build Observatory -> Telescope -> Instrument, return it."""
    observatory, _ = client.get_or_create(
        "/api/observatories/",
        {"name": OBSERVATORY_NAME, "utc_offset": 0, "tz_name": "UTC"},
        match=lambda o: o["name"] == OBSERVATORY_NAME,
    )
    telescope, _ = client.get_or_create(
        "/api/telescopes/",
        {"name": TELESCOPE_NAME, "latitude": 0.0, "longitude": 0.0,
         "elevation": 0.0, "observatory": observatory["url"]},
        match=lambda t: t["name"] == TELESCOPE_NAME
        and t["observatory"] == observatory["url"],
    )
    instrument, _ = client.get_or_create(
        "/api/instruments/",
        {"name": INSTRUMENT_NAME, "telescope": telescope["url"]},
        match=lambda i: i["name"] == INSTRUMENT_NAME
        and i["telescope"] == telescope["url"],
    )
    return instrument
...
def build_photometry(client, instrument, rows, known_bands):
...
def ensure_tag_exists(client, tag):
    """Hard-fail unless a TransientTag named `tag` already exists in the DB."""
    page = client.get("/api/transienttags/?limit=5000")
    if not any(t["name"] == tag for t in page.get("results", [])):
        die(f"tag {tag!r} does not exist in the database")
...
def main():
    ...
    parser.add_argument("transient_list", help="path to the transient list (cid ra dec index)")
    parser.add_argument("lightcurve_dir", help="directory holding the <index>_lc.txt files")
    parser.add_argument("tag", help="a TransientTag name that must already exist in the DB")
    ...
    ensure_tag_exists(client, args.tag)

    instrument = ensure_instrument(client)
    known_bands = load_known_bands(client, instrument)
    print(f"[setup] instrument={instrument['name']}  "
          f"known bands={sorted(known_bands) or 'none'}  tag={args.tag}")
